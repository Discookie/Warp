...
TEST_F(SceneMessengerFixture, InitTest) {
  EXPECT_FALSE(scene_messenger.get_scene());
}
...
protected:
  ...
  void render_scene(SceneMessenger &messenger,
                    const ALLEGRO_EVENT &event) override {}

public:
  explicit TestScene(unsigned int id) : _id(id) {}
...
protected:
  ...
  [[nodiscard]] TestScene *
  extract_from_scene_manager(const std::string &scene_to_remove) {
    return dynamic_cast<TestScene *>(
        scene_manager.remove_scene(scene_to_remove).value().get());
  }
...
TEST_F(SceneManagerFixture, InitTest) {
  ...
  ASSERT_EQ(test_scenes[0]->getId(), 0);
  ASSERT_EQ(test_scenes[1]->getId(), 1);
  ASSERT_EQ(test_scenes[2]->getId(), 2);
  ASSERT_EQ(test_scenes[3]->getId(), 3);
...
TEST_F(SceneManagerFixture, RemoveEmptySceneTest) {
  EXPECT_FALSE(scene_manager.remove_scene(""));
}
